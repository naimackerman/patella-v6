...
For downstream KL modeling, the current aligned sclerosis implementation uses
the best manual-label binary texture-only classifier with a validation-tuned
global threshold. The thresholded binary grade is stored in
`scl_grade_medial` and `scl_grade_lateral` as `0=none, 1=present`; the remaining
texture features stay continuous. Final downstream feature extraction uses model
predictions for all images, including manually reviewed images, to avoid leaking
manual sclerosis labels into the KL classifier evaluation.
...
Implementation note: binary pseudo-label fine-tuning is allowed as a separate
semi-supervised branch to use more of the image set. Pseudo rows generated by
the manual binary texture-only teacher are marked `high_conf_model` and are
down-weighted during training, while manual-review validation and test splits
remain the acceptance criterion. This branch should not replace the manual-only
binary baseline unless it improves held-out manual macro-F1/AUC and preserves
balanced recall for both `none` and `present`.

**Output features**:
- `scl_grade_medial`: Sclerosis grade for medial subchondral region. In the
  primary severity endpoint this is 0=none, 1=mild, 2=significant; in the
  current downstream binary endpoint this is 0=none, 1=present.
- `scl_grade_lateral`: Sclerosis grade for lateral subchondral region, using
  the same endpoint coding as `scl_grade_medial`
